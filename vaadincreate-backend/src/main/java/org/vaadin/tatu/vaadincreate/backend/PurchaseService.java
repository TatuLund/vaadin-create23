 ...
 * Provides operations for creating and querying purchase requests.
 */
@NullMarked
public interface PurchaseService {

    /**
     * Creates a new pending purchase from a cart.
     * This method:
     * - Creates a Purchase with status PENDING
     * - Snapshots the delivery address
     * - Creates PurchaseLines with current product prices
     * - Does NOT modify product stock
     *
     * @param cart the cart containing products and quantities
     * @param address the delivery address for this purchase
     * @param requester the user making the purchase request
     * @param defaultApproverOrNull the approver to assign, or null to use default
     * @return the created purchase
     */
    Purchase createPendingPurchase(Cart cart, Address address, User requester, @Nullable User defaultApproverOrNull);
     ...
     */
    List<@NonNull Purchase> findMyPurchases(User requester, int offset, int limit);
     ...
     */
    List<@NonNull Purchase> findPendingForApprover(User approver, int offset, int limit);
