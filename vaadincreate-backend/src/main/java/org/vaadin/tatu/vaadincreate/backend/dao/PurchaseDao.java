...
/**
 * Data access object for managing purchases.
 * Provides CRUD operations and queries for Purchase entities.
 */
@NullMarked
...
public class PurchaseDao {
     ...
     *
     * @param purchase the purchase to save or update
     ...
     *
     * @param requester the user who created the purchases
     * @param offset the starting offset for pagination
     * @param limit the maximum number of results
     * @return list of purchases
     */
    public List<@NonNull Purchase> findByRequester(User requester, int offset, int limit) {
        ...
        if (result == null) {
            throw new IllegalStateException("Result of findByRequester is null");
        }
        return result;
    }
     ...
     * @param status the purchase status to filter by
     * @param offset the starting offset for pagination
     * @param limit the maximum number of results
     * @return list of purchases
     */
    public List<@NonNull Purchase> findByApproverAndStatus(User approver, PurchaseStatus status, int offset, int limit) {
        Objects.requireNonNull(approver, "Approver must not be null");
        Objects.requireNonNull(status, "Status must not be null");
        logger.info("Fetching Purchases by approver: ({}) and status: {}", approver.getId(), status);
        var result = HibernateUtil.inSession(session -> {
            return session.createQuery(
        ...
        if (result == null) {
            throw new IllegalStateException("Result of findByApproverAndStatus is null");
        }
        return result;
    }
    ...
    public long countByApproverAndStatus(User approver, PurchaseStatus status) {
        Objects.requireNonNull(approver, "Approver must not be null");
        Objects.requireNonNull(status, "Status must not be null");
        logger.info("Counting Purchases by approver: ({}) and status: {}", approver.getId(), status);
        var result = HibernateUtil.inSession(session -> {
            return session.createQuery(
    ...
    public long countAll() {
        ...
        var result = HibernateUtil.inSession(session -> {
            return session.createQuery("select count(p) from Purchase p", Long.class)
                    .uniqueResult();
        });
        return result != null ? result : 0L;
    }
    ...
    public UserSupervisor updateUserSupervisor(UserSupervisor mapping) {
        Objects.requireNonNull(mapping, "UserSupervisor mapping must not be null");
